...
# 摄像头参数设置（俯视）
cam_target = [0.5, 0.0, 0.0]   # 观察区域中心
cam_eye = [0.5, 0.0, 1.0]      # 相机位置（正上方）
up_vector = [0, 1, 0]
...
rgb_array = img[2]  # 这是一个 width x height x 4 的图像数组（RGBA）
...
cube_ids = []
base_x, base_y = 1.2, 0   # 中央区域
layers = 4  # 堆叠层数
per_layer = 20  # 每层放几个

for layer in range(layers):
    z = 0.02 + layer * 0.03  # 每层稍微高一点
    for i in range(per_layer):
        x_offset = random.uniform(-0.05, 0.05)
        y_offset = random.uniform(-0.05, 0.05)
        pos = [base_x + x_offset, base_y + y_offset, z]
